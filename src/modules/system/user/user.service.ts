 ...
 * @Description: Quản lý người dùng service
...
export class UserService {
  ...
  /* Có được người dùng thông qua tên người dùng, loại bỏ hệ thống treo và Xoá Quang học để đăng nhập */
  ...
  /* Có được người dùng thông qua tên người dùng, loại bỏ Xoá của */
  ...
  //vượt qua id Tìm tất cả thông tin của người dùng
  ...
  /* vượt qua id Kiểm tra tất cả thông tin của người dùng, loại bỏ hệ thống treo và Xoá của */
  ...
  async updateUser(reqUpdateUserDto: ReqUpdateUserDto) {
    ...
    if (
      ...
      await this.redis.set(`${USER_VERSION_KEY}:${reqUpdateUserDto.userId}`, 2); //Điều chỉnh phiên bản mật khẩu và buộc người dùng phải đăng nhập lại
    }
  }
  ...
  async resetPwd(userId: number, password: string, updateBy: string) {
    ...
    if (await this.redis.get(`${USER_VERSION_KEY}:${userId}`)) {
      await this.redis.set(`${USER_VERSION_KEY}:${userId}`, 2); //Điều chỉnh phiên bản mật khẩu và buộc người dùng phải đăng nhập lại
    }
  }
  ...
  async updateSelfPwd(reqUpdateSelfPwd: ReqUpdateSelfPwd, userName: string) {
    ...
    if (password !== user.password) throw new ApiException('Lỗi mật khẩu cũ');
    ...
    if (await this.redis.get(`${USER_VERSION_KEY}:${user.userId}`)) {
      await this.redis.set(`${USER_VERSION_KEY}:${user.userId}`, 2); //Điều chỉnh phiên bản mật khẩu và buộc người dùng phải đăng nhập lại
    }
  }

  /* Nhập Excel Batch chèn người dùng */
  async insert(data: any, userName: string) {
    const userArr: User[] = [];
    for await (const iterator of data) {
      let user = new User();
      if (!iterator.userName || !iterator.password || !iterator.nickName)
        throw new ApiException('Tài khoản người dùng, biệt danh người dùng và mật khẩu người dùng không thể trống');
      const one = await this.findOneByUsername(iterator.userName);
      if (one) throw new ApiException('Tài khoản người dùng đã tồn tại, vui lòng kiểm tra');
